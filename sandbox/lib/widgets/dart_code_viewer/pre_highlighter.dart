...
class DartSyntaxPreHighlighter extends SyntaxPreHighlighter {
  ...
  bool _generateSpans() {
    ...
    while (!_scanner.isDone) {
      ...
      // Check for method calls (word.word followed by parentheses)
      if (_scanner.scan(RegExp(r'\w+\.\w+\s*\('))) {
        final match = _scanner.lastMatch![0]!;
        final dotIndex = match.indexOf('.');
        final openParenIndex = match.indexOf('(');
        ...
        _spans.add(
          _HighlightSpan(
            ...
            _scanner.lastMatch!.start,
            _scanner.lastMatch!.start + dotIndex,
          ),
        );
        ...
        _spans.add(
          _HighlightSpan(
            _HighlightType.punctuation,
            _scanner.lastMatch!.start + dotIndex,
            _scanner.lastMatch!.start + dotIndex + 1,
          ),
        );
        ...
        _spans.add(
          _HighlightSpan(
            _HighlightType.method,
            _scanner.lastMatch!.start + dotIndex + 1,
            _scanner.lastMatch!.start + openParenIndex,
          ),
        );
        ...
        _spans.add(
          _HighlightSpan(
            _HighlightType.punctuation,
            _scanner.lastMatch!.start + openParenIndex,
            _scanner.lastMatch!.start + openParenIndex + 1,
          ),
        );
      ...
      // Check for properties (word.word without parentheses)
      if (_scanner.scan(RegExp(r'\w+\.\w+(?!\s*\()'))) {
        final match = _scanner.lastMatch![0]!;
        final dotIndex = match.indexOf('.');
