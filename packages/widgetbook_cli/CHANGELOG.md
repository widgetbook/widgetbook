...
- **FEAT**: Accept workspace API keys (`widgetbook_ws_...`) in `cloud build push` and `cloud review skip`, together with a new `--project` option that names the project.
- **FEAT**: Add `cloud project create`, which creates a project in a Widgetbook Cloud workspace using a workspace API key.
- **FEAT**: Show `--api-url` in the usage of `cloud build push`, `cloud review skip` and `cloud project create`.

## 4.0.0-beta.14

- **FIX**: Migrate to the `platform` package's 3.2 `NativePlatform` API. ([#2033](https://github.com/widgetbook/widgetbook/pull/2033))
- **CHORE**: Align version with `widgetbook` so both packages share `4.0.0-beta.14`.
- **CHORE**: Update the `widgetbook` version installed by `widgetbook init` to `4.0.0-beta.14`.
...
- **FIX**: Append a missing trailing slash to the `--api-url` value of `cloud build push` and `cloud review skip`, so custom API URLs work without a trailing slash. ([#1976](https://github.com/widgetbook/widgetbook/pull/1976), [#1995](https://github.com/widgetbook/widgetbook/pull/1995))
- **CHORE**: Update the `widgetbook` version installed by `widgetbook init` to `4.0.0-beta.11`.
...
- **FIX**: Update the `widgetbook` version installed by `widgetbook init` to `4.0.0-beta.6`.
