...
Since the [order of addons](/addons#order-of-addons) matters, here are some guidelines to follow when using the `AlignmentAddon`:
...
The `AlignmentAddon` is supported in the [Multi Snapshot Reviews](/cloud/reviews/multi-snapshot) via the `AlignmentAddonConfig`. Here's how to configure it:
