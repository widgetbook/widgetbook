...
<Warning>
  ...
  introduced at any minor version. It will replace the [Device Frame
  Addon](/addons/device-frame-addon) in the near future.
</Warning>
...
The `ViewportAddon` is supported in the [Multi Snapshot Reviews](/cloud/reviews/multi-snapshot) via the `ViewportAddonConfig`. Here's how to configure it:
