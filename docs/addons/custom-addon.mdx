...
This is a list of [fields](/configure/fields) that describe how the addon will be represented in both:
...
This can be done by using the `valueOf` helper method to extract the values from the query group by giving it **the field's name**.
...
This method is used to be able to convert `Modes` to a serializable `QueryGroup`, so that it can be stored in the URL.
...
This method is responsible for building the story widget with the addon applied.
