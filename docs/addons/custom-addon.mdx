...
The constructor for `AlignmentAddon` takes two parameters. `alignment` is a required
`Alignment` parameter, `initialAlignment` is an optional `Alignment?` that, if provided,
will be used as the initial value of the addon. If it is not provided, `initialAlignment`
