...
The Theme Addon lets you inject a theme widget into your tree, to make your stories styled and try them out with different themes.
...
Each addon has a corresponding [Mode](/addons/modes) to lock its value in scenarios. For this addon, use the theme mode matching your addon variant:
  ...
  scenarios: [
    _Scenario(
      ...
      modes: [
        ...
        CupertinoThemeMode('Dark', CupertinoThemeData(brightness: Brightness.dark)), // [!code highlight]
        ThemeMode('Dark', myAppTheme, myAppThemeBuilder), // [!code highlight]
      ]
    ),
  ],
)
