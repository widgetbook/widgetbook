...
To evaluate rules against a scenario, see [Accessibility Guidelines](/testing/accessibility); to compare the tree across a pull request, see [Accessibility regressions](/cloud/reviews/accessibility-regressions) in Widgetbook Cloud.
</Info>
...
Each addon has a corresponding [Mode](/addons/modes) to lock its value in scenarios. For this addon, use `SemanticsMode`:
