...
Since the [order of addons](/addons#order-of-addons) matters, here are some guidelines to follow when using the `TextScaleAddon`:
...
The `TextScaleAddon` is supported in the [Multi Snapshot Reviews](/cloud/reviews/multi-snapshot) via the `TextScaleAddonConfig`. Here's how to configure it:
