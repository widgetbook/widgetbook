...
@UseCase(type: DurationText, name: 'Default')
Widget buildDurationTextUseCase(BuildContext context) {
  return DurationText(duration: context.knobs.duration(label: 'duration')); // [!code highlight]
}
