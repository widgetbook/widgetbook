...
@UseCase(type: CircularProgressIndicator, name: 'Distinct')
...
@UseCase(type: CircularProgressIndicator, name: 'Flexible')
Widget buildCircularProgressIndicatorUseCase(BuildContext context) {
  return CircularProgressIndicator( // [!code highlight]
    ...
    backgroundColor: Colors.amber, // [!code highlight]
  ); // [!code highlight]
}
```
