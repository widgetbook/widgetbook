...
Snapshots is the billing unit of the builds in Widgetbook Cloud. A snapshot is a [addon configuration](/cloud/reviews/multi-snapshot#multi-snapshot-for-addons) and [knob configuration](cloud/reviews/multi-snapshot#multi-snapshot-for-knobs) for a use-case inside a build. For example:
...
When you create a build, Widgetbook Cloud will automatically create a default configuration for each use-case. This configuration is the one that is shown when the use-case is first opened in Widgetbook.
