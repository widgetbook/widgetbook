...
A pull request that touches no UI code uploads no build, so Widgetbook never reports the review check for it.
With the ruleset active, those pull requests stay blocked with nothing to click.

To resolve the check on them, let your CI declare that no build is coming:

<Steps>
  <Step title="Allow CI to skip the review">
    In your project's *General* settings, enable *Allow CI to skip the review check*.

    This is off by default because it lets anyone who can run CI with the project API key waive the review, so keep your workflow files protected accordingly.
  </Step>
