...
<Steps>
  ...
  <Step title="Call the CLI when no build is uploaded">
    In the branch of your pipeline that skips the Widgetbook build, run [`cloud review skip`](/cli/commands/cloud-review-skip) instead:
    ...
    Pass the head commit of the pull request, not the merge commit that your CI checks out by default.
  </Step>
</Steps>
