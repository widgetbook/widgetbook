...
<Steps>
  ...
  <Step title="Add API Key as a Secret">
    ...
    The key is shown only once, so add it as `WIDGETBOOK_API_KEY` to your [GitHub repository's secrets](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions#creating-secrets-for-a-repository) right away.
      ...
      widgetbook-cloud-hosting:
        ...
        steps:
          ...
          - name: Push Widgetbook Build
            working-directory: widgetbook
            run: widgetbook cloud build push --api-key ${{ secrets.WIDGETBOOK_API_KEY }} --project my-app
    ```
