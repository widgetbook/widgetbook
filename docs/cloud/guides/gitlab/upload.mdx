...
<Steps>
  ...
  <Step title="Add API Key as a Variable">
    ...
    The key is shown only once, so add it as `WIDGETBOOK_API_KEY` to your [GitLab's CI/CD Variables](https://docs.gitlab.com/ee/ci/variables/#for-a-project) right away.
    Make sure you **uncheck "Protect variable" checkbox**, to allow the variable to be accessible from feature branches.
  ...
  <Step title="Create GitLab CI/CD Pipeline">
    ...
    widgetbook_cloud_hosting:
      ...
      script:
        ...
        - widgetbook cloud build push --api-key "$WIDGETBOOK_API_KEY" --project my-app
    ```
...
widgetbook:
  script:
    - # Previous scripts...
    - widgetbook cloud build push --api-key "$WIDGETBOOK_API_KEY" --project my-app // [!code --]
    - | // [!code ++]
