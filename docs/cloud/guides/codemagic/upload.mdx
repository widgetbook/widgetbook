...
<Steps>
  <Step title="Add API Key as a Secret">
    ...
    The key is shown only once, so add it as `WIDGETBOOK_API_KEY` under the `widgetbook_credentials` group in your [Codemagic project's environment variables](https://docs.codemagic.io/yaml-basic-configuration/configuring-environment-variables/#configuring-environment-variables) right away.
      ...
      widgetbook-cloud-hosting:
        ...
        scripts:
          ...
          - name: Install Widgetbook CLI
            script: dart pub global activate widgetbook_cli {{ versions.cli }}

          - name: Push Widgetbook Build
            working_directory: widgetbook
            script: widgetbook cloud build push --api-key $WIDGETBOOK_API_KEY --project my-app
    ```
