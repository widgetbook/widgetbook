...
A violation is a broken [accessibility guideline](/testing/accessibility), such as a tap target below the minimum size or a button without a label. `testWidgetbook` evaluates the guidelines for every scenario and uploads the result with the build.
...
A violation is identified by its guideline and the element that broke it, independent of the element's position and the measured values. A tap target that moved, or that grew but is still too small, stays the same violation instead of showing up as one resolved plus one new.
...
Which guidelines run is configured in your Widgetbook project, not in Widgetbook Cloud. See [Accessibility Guidelines](/testing/accessibility) for the default set and for writing your own.
