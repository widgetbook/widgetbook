...
<Info>
  **Coming soon.**
  The Figma design will be visible inside Widgetbook Cloud, diffed directly against the Flutter implementation.
  A review compares two Flutter builds today, so it catches regressions but not a deviation from the design.
