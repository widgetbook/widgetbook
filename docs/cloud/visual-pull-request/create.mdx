...
- **Head build**: checks out and uploads the pull request's head commit `z` (not the merge commit `m`), so Widgetbook Cloud can match the build to the pull request's head.
  Passing `--branch` and `--commit` explicitly is required, because the checked-out head commit is in a detached `HEAD` state.
