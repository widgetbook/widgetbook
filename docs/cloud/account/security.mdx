...
Widgetbook is **ISO 27001 certified**. In the following, you can find the most important security information. For more details, please check our (['Trust Center'](https://trust.widgetbook.io/)). 
...
We strongly encourage developers to use [mocked data](/stories/mocking) instead of real user data when using Widgetbook. This ensures that **no sensitive or personal information is stored or displayed** in Widgetbook or Widgetbook Cloud — neither in the rendered snapshots nor in the accessibility semantics (labels, hints, tooltips) captured alongside them.
...
- A serialized Flutter **semantics tree** per scenario, used to power accessibility comparisons in Cloud Reviews. The exact set of serialized fields is defined in the OSS package (see [`semantics_data_extension.dart`](https://github.com/widgetbook/widgetbook/blob/v4/packages/widgetbook/lib/src/test/semantics/semantics_data_extension.dart)).

### Telemetry

The Widgetbook OSS package reports anonymous usage information to help us improve the product. Telemetry is **separate from Cloud uploads**, contains no personal data, and can be disabled. See [Telemetry](/telemetry) for the collected fields and opt-out instructions.
