...
A code review shows a diff of the source, not the rendered UI. To judge a UI change, a reviewer has to check out the branch, run the app, navigate to the screen, and reproduce every state and configuration by hand. Most reviewers skip that and review only the code.
