...
- Approvals from non-required reviewers alone are not sufficient to approve the pull request.
  At least one required reviewer must approve.
...
<Info>
  Requiring a designer on every pull request is one of three policies.
  See [Decide when to review](/cloud/reviews/when-to-review).
</Info>
