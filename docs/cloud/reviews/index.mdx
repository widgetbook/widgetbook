...
A review compares the `base` and the `head` build of a pull request and reports three kinds of findings:

1. [**Visual changes**](/cloud/visual-pull-request/overview): differences between the snapshots of a scenario.
1. [**Accessibility regressions**](/cloud/reviews/accessibility-regressions): differences between the accessibility trees of a scenario, such as a dropped label or a changed role.
1. [**Accessibility violations**](/cloud/reviews/accessibility-violations): broken [accessibility guidelines](/testing/accessibility), split into the ones introduced, kept and resolved by the pull request.
...
1. [**Accessibility Regressions**](/cloud/reviews/accessibility-regressions): Catch changes to the accessibility tree, such as missing labels, changed roles and reordered elements, that a visual review cannot see.
