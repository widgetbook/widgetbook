...
<Steps>
  ...
  <Step title="Get your API key">
    ...
    The key is shown only once, so store it somewhere safe.
  </Step>

  <Step title="Create the project">
    Creating the project is a one-time setup step, so skip it if the project already exists in Widgetbook Cloud.

    ```bash
    widgetbook cloud project create --api-key WORKSPACE_API_KEY --project my-app
    ```

    See [`cloud project create`](/cli/commands/cloud-project-create) for details.
  </Step>
  ...
  <Step title="Push the build to Widgetbook Cloud">
    ...
    If you rename the project in Widgetbook Cloud, update `--project` as well.
  </Step>
</Steps>

<Info>
Project API keys from the Widgetbook Cloud **project settings page** still work.
