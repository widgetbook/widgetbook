...
resolving in the test environment. See [Fonts](/essentials/fonts) for the cause and
the fix.

## Accessibility Guideline Violations

For every scenario, `testWidgetbook` evaluates a list of `WidgetbookGuideline`s
against the rendered widget tree and records the failures in the metadata's
`violations` field. Native guidelines report each offending node individually,
including its on-screen `rect` (physical pixels, matching the screenshot):
...
When `guidelines` is omitted, `WidgetbookGuidelines.recommended` is used:
native Android (48dp) and iOS (44dp) tap-target size, native labeled-tappable,
and text contrast. Contrast is adapted from Flutter's built-in guideline — it
samples the rendered image, is approximate, and reports a coarse text `reason`
(no per-node `rect`), so treat it as advisory.

### Custom guidelines

Override the list to add your own. Implement `WidgetbookGuideline` for fully
structured, per-node violations, or wrap any Flutter
[`AccessibilityGuideline`](https://api.flutter.dev/flutter/flutter_test/AccessibilityGuideline-class.html)
with `FlutterGuideline` (text-only):
...
Future<void> main() async {
  await testWidgetbook(
    ...
    guidelines: [
      ...
      MyBrandGuideline(), // implements WidgetbookGuideline
    ],
  );
}
```

A guideline that throws during evaluation is recorded as a violation with a
