...
<Info>
  In **v4-alpha**, if you have more than one story, you can share the `argsBuilder`
  function between them by defining a top-level function named `argsBuilder`
  with the same signature, and the generator will use it as the default parameter.

  ```dart
  Widget argsBuilder(BuildContext context, CustomInputs args) {
    return CoolWidget(
      text: args.number.resolve(context).toString(),
    );
  }

  final $First = CoolWidgetStory();
  final $Second = CoolWidgetStory();
  ```
</Info>
...
1. Add the following:
   ...
   ```json
   {
     "Widgetbook 4 Story": {
       "scope": "dart",
       "prefix": "stry",
       "description": "Creates a new Widgetbook 4 story",
       "body": [
         "import 'package:widgetbook/widgetbook.dart';",
         "",
...
1. Abbreviation: `stry`
1. Description: `Creates a new Widgetbook 4 Story`
1. Template Text:
