...
To catalog the `UserTile` widget, you need to provide the `UserProvider` to the Widget tree or reduce the dependency of the `UserTile` by property extraction so its value can be easily provided.
