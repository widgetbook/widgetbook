...
You will get an error because the `UserProvider` is missing from the Widget tree.
...
To catalog the `UserTile` widget, you need to provide the `UserProvider` to the Widget tree or extract the property of the `UserTile` so it can be easily provided.

<Info>
  For information about properly mock a widget or screen please refer to the [Mocking](/guides/mocking) guide.
</Info>