...
<Info>
  In the following code, instead of passing `text` as a constant value, you can
  use a [knob](/knobs) to change the text value in the Widgetbook UI, and see
  how your button behaves with extra-long text, or with an empty text.
</Info>

```dart title="widgetbook/lib/button.use_cases.dart"
import 'package:flutter/material.dart';
