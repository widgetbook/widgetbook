...
Adding a story for a widget that has external dependencies can be challenging.
...
2. Mock `UserProvider` in the `setup` function as follows
   ...
   final $Primary = _Story(
     setup: (context, child, args) {
       ...
       when(() => mockProvider.user).thenReturn('Mocked User');
