...
### One file per constructor

Each `.stories.dart` file targets a single constructor via its `Meta`. To catalog both the default and a named constructor, create separate story files:

```
lib/
├── custom_button.stories.dart        # Meta<CustomButton>()
└── custom_button_icon.stories.dart   # Meta<CustomButton>(constructor: CustomButton.icon)
```

Both appear as separate components in the navigation tree. By default, the named constructor variant is labeled `CustomButton.icon`. You can override this with the `name` parameter.
