...
To catalog several constructors of the same widget under a single component, declare more than one `Meta<TWidget>` in the same `.stories.dart` file. The variable named `meta` is the canonical one; it provides the component's `name`, `path`, and `docsBuilder`. Additional metas target other constructors:
