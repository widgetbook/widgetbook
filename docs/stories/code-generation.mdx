...
The generator inspects the named constructor's parameters (not the default constructor's) and produces `_Args` with fields matching that constructor. The generated builder calls `CustomButton.icon(...)` automatically.
...
The default constructor's args and story typedefs remain `_Args` and `_Story`. Each additional meta gets typedefs prefixed by its constructor name in PascalCase: `iconMeta` (constructor `icon`) → `_IconArgs` / `_IconStory`. The generator inspects each meta's `constructor:` tear-off to derive the prefix, so the variable name itself (`iconMeta` vs anything else) is convention rather than requirement.

A file that declares only a single `Meta`, whether targeting the default constructor or a named one, keeps the original output shape: a single `_Args` / `_Story` pair and a component labeled `CustomButton.icon` by default (overridable via the `name` parameter).
...
When a file declares additional metas to [group multiple constructors](#grouping-multiple-constructors), each non-default meta generates its own `_{Constructor}Args` and `_{Constructor}Story` typedefs (and corresponding `{Widget}{Constructor}Args` / `{Widget}{Constructor}Story` classes) alongside the ones above. The `_Component` typedef widens to `Component<TWidget, StoryArgs<TWidget>>` so a single component can hold stories with heterogeneous args.
