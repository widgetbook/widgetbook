...
For a meta targeting a [named constructor](#named-constructors), every declaration above is prefixed with the constructor name in PascalCase: a `Meta<Widget>(constructor: Widget.icon)` produces `_IconArgs`, `_IconStory`, `_IconScenario`, `_IconDefaults`, and a `WidgetIconComponent`. When a file [groups multiple metas](#grouping-multiple-constructors), each non-default meta contributes its own prefixed `_{Constructor}Args` and `_{Constructor}Story` typedefs alongside the default `_Args` / `_Story`, and the shared `_Component` widens to `Component<TWidget, StoryArgs<TWidget>>` so a single component can hold stories with heterogeneous args.
