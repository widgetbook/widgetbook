...
A compiled AOT version of the CLI is downloadable [here](https://firebasestorage.googleapis.com/v0/b/widgetbook-cloud.appspot.com/o/tools%2Fcli-v1.aot?alt=media), but it's understandable if you'd like to check the source code first and compile it yourself. 
...
Since the CLI is AOT-compiled, the `cli.aot` file requires the [`dartaotruntime`](https://dart.dev/tools/dartaotruntime) to run the file.
The `dartaotruntime` is located at `<PathToRootFolderOfSdk>/flutter/bin/cache/dart-sdk/bin` and can be executed like this:

```bash
dartaotruntime cli.dart
```
