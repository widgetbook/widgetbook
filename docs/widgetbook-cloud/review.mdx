...
- Use the `widgetbook` in version `v3.0.0-beta.22` CLI from the [widgetbook_cli](https://pub.dev/packages/widgetbook_cli) package. For instructions see [Hosting](/widgetbook-cloud/hosting).
...
1. Go to https://github.com/user-name/repository-name/settings/hooks. Make sure to replace `user-name` and `repository-name` with your appropriate values.
2. Click `Add webhook`
