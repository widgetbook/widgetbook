...
1. [Concept of reviews](/widgetbook-cloud/reviews#understanding-reviews)
2. [Publish a review](/widgetbook-cloud/reviews#publishing-a-review)
4. [Review Statuses](/widgetbook-cloud/reviews#review-statuses)
5. [Set up addons and knobs](/widgetbook-cloud/reviews#how-to-set-up-addons-and-knobs)
6. [Add a Figma design link](/widgetbook-cloud/reviews#how-to-add-a-figma-design-link)
...
1. Make sure to incorporate the [widgetbook_generator package](https://pub.dev/packages/widgetbook_generator) version `{{ versions.generator }}`. No additional configuration needed.
2. Integrate the [widgetbook package](https://pub.dev/packages/widgetbook) version `{{ versions.widgetbook }}`. No extra setup required.
3. Employ the CLI from [widgetbook_cli package](https://pub.dev/packages/widgetbook_cli).
4. Set up our GitHub app.
5. Upload a build of your base branch, e.g. main. See [how to upload a build](https://docs.widgetbook.io/widgetbook-cloud/hosting).
6. Add new or change existing Widgetbook use cases in your feature branch. Note: Without changed use cases, the review feature won't work, as there would be nothing to review.
7. Optionally, [add addons and knobs](/widgetbook-cloud/reviews#how-to-set-up-addons-and-knobs) as well as [Figma design links](/widgetbook-cloud/reviews#how-to-add-a-figma-design-link).
8. Upload a build for the feature branch with changed Widgetbook use cases.
9. Publish a review using this command: `widgetbook publish --api-key KEY --base-branch BASE-BRANCH`.
