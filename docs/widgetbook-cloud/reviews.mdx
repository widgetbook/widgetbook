...
1. **Disable animations**: by providing a constant value to the parameter that controls the animation, you can disable the animations during the visual comparison.
...
Currently, only the default use-case configuration is considered for the visual comparison. If you have multiple addons/knobs configurations, we have good news for you! **We are working on supporting this in the near future**.
