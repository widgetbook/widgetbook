...
MCP server requires Widgetbook 3.17.0-beta or later:

```bash
flutter pub add widgetbook 3.17.0-beta
```
...
### Navigation
- **`search_use_cases`** - Search for use cases using patterns (supports glob patterns like `button*`, `*input*`, `forms/**`)
- **`get_use_case_count`** - Get overview statistics (total use cases and components) without listing everything
- **`list_use_cases`** - Get complete Widgetbook structure (⚠️ WARNING: For large projects, prefer search_use_cases to avoid context bloat)
