...
See [Assets](/essentials/assets) for how to set that package up so the same font
...
described in [Assets](/essentials/assets).

<Warning>
  Do not point at another package's font file with a relative path such as
  `../design_system/fonts/MyFont-Regular.otf`. `flutter test` reads it straight
  from disk, so snapshots look correct, but `flutter build` does not bundle the
  file and the font silently falls back in the built Widgetbook app.
</Warning>
