...
one might add `@WidgetbookApp` to the `App` Widget defined in `ui_catalogs.dart`.
...
[package:widgetbook_generator](/widgetbook-generator/overview) will then create a new file `ui_catalogs.widgetbook.dart` next to the `ui_catalogs.dart` file. The resulting app structure will look like this:
...
You can also set `isDefault` to true if you want the theme to be the default on startup.
