...
In addition, different Flutter states of a button (e.g. `focused` or `hovered`) might be resembled in Figma as additional variants. 
...
>
  <TabItem value="generator">
    ...
    As a developer you want to specify the type of the catalogued component (`Container`) instead of the one that might be returned by the builder method (`Column`).
    ...
    ### Navigation structure with the generator

    The generator creates the navigation tree based on the file location of the component that has been specified for the `type` parameter of the `@WidgetbookUseCase` annotation so the navigations structure of the Widgetbook will resemble the structure of your application or package.
