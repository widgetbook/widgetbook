...
>
  ...
  1- To add component in generator approach, you can start by adding use-cases with the generator is based on the `@WidgetbookUseCase` annotation.
...
3- In addition, `@WidgetbookUseCase` requires two mandatory parameters `type` and `name` that allow you to customize how the use-case is shown in the navigation of Widgetbook.
    ...
    4- Run the `build_runner` again via `flutter pub run build_runner build` or `flutter pub run build_runner watch` to update the Widgetbook with the newly created use-case.
...
<Image
  zoom
  src="/assets/screenshot/first_component.png"
  alt="First Component"
